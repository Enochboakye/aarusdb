...
function SuspectsPageContent() {
  ...
  // Initialize filter state directly from searchParams ONCE
  ...
  }, [filter]); // fetchSuspects depends on the local filter state

  useEffect(() => {
    fetchSuspects();
  }, [fetchSuspects]); // Re-fetch when fetchSuspects (i.e., filter) changes

  // Effect to update local filter state if URL search parameter 'q' changes externally
  useEffect(() => {
    const queryParamValue = searchParams.get('q') || '';
    if (queryParamValue !== filter) {
      setFilter(queryParamValue);
    }
  }, [searchParams, filter]);
    ...
    <>
      {loading ? (
        ...
        <DataTable
          ...
          onFilterChange={setFilter} // Directly set the local filter state
          filterValue={filter}
          meta={{
